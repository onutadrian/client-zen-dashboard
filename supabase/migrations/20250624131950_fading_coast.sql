...
BEGIN
  ...
  -- Check for "Public can check specific invite code" policy
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    ...
    EXECUTE 'CREATE POLICY "Public can check specific invite code" 
      ON public.invite_codes 
      FOR SELECT 
      TO public 
      USING (
...
-- Add delete policy for invite codes (admin only)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    ...
    EXECUTE 'CREATE POLICY "Admins can delete invite codes" 
      ON public.invite_codes 
      FOR DELETE 
      TO authenticated 
      USING (
