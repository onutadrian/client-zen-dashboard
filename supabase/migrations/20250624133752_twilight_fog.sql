/*
  # Fix User Roles and Invite Codes System

  1. New Types
    - `user_role` enum type with values 'admin' and 'standard'
  
  2. Changes
    - Add `role` column to profiles table
    - Create invite_codes table for managing user invitations
    - Set up proper RLS policies for invite_codes
  ...
  3. Security
    ...
    - Allow public checking of specific invite codes
*/
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invite_codes' AND policyname = 'Admins can create invite codes') THEN
    DROP POLICY "Admins can create invite codes" ON public.invite_codes;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invite_codes' AND policyname = 'Admins can delete invite codes') THEN
    DROP POLICY "Admins can delete invite codes" ON public.invite_codes;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invite_codes' AND policyname = 'Admins can view all invite codes') THEN
    DROP POLICY "Admins can view all invite codes" ON public.invite_codes;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invite_codes' AND policyname = 'Public can check specific invite code') THEN
    DROP POLICY "Public can check specific invite code" ON public.invite_codes;
  END IF;
END $$;
...
-- Update or create handle_new_user function to support roles from invite codes
